...
function [Error]=DPARSFA_run_XNAT_v5(AutoDataProcessParameter,WorkingDir,SubjectListFile,IsAllowGUI)
...
if (AutoDataProcessParameter.IsRealign==1)
    ...
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        ...
        save([AutoDataProcessParameter.DataProcessDir,filesep,'RealignParameter',filesep,AutoDataProcessParameter.SubjectID{i},filesep,'HeadMotion.mat'],'HeadMotion');
        ...
        fid = fopen([AutoDataProcessParameter.DataProcessDir,filesep,'RealignParameter',filesep,AutoDataProcessParameter.SubjectID{i},filesep,'HeadMotion.tsv'],'w');
        ...
        fid = fopen([AutoDataProcessParameter.DataProcessDir,filesep,'RealignParameter',filesep,AutoDataProcessParameter.SubjectID{i},filesep,'ExcludeSubjectsAccordingToMaxHeadMotion.txt'],'at+');
    ...
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        ...
        for i=1:AutoDataProcessParameter.SubjectNum
            ...
            if (AutoDataProcessParameter.Covremove.CSF.IsRemove==1) && strcmpi(AutoDataProcessParameter.Covremove.CSF.Method,'Mean')
                ...
                elseif strcmpi(AutoDataProcessParameter.Covremove.CSF.Mask,'Segment') % (Yes - Gao: CSF segment) modified by Gao on 6/15/2022
                    % get existing CSF mask
                    [maskCSF, maskVox, maskHeader]=y_ReadRPI([AutoDataProcessParameter.DataProcessDir,filesep,'Masks',filesep,'SegmentationMasks',filesep,'FunSpace_ThrdMask_',AutoDataProcessParameter.SubjectID{i},'_CSF.nii']); 
                    cnt = 0; % count of erodsion
                    T = AutoDataProcessParameter.Covremove.CSF.MaskThreshold;
                    vVox = maskVox(1)*maskVox(2)*maskVox(3)/1000; % in cm3
                    kk = 1;

                    % get the largest ROI in CSF mask (=ventricle mask)
                    st = regionprops3(maskCSF>0, 'Volume', 'VoxelIdxList', 'Centroid'); 
                    [nCSF, indCSF] = sort(st.Volume, 'descend'); 
                    nVentricle = nCSF(kk); 
                    vVentricle = round(vVox*nVentricle); % in cm3
                    cVentricle = st.Centroid(indCSF(kk),3)/maskHeader.dim(3); 
                    disp(['Ventricle (eroded ' num2str(cnt) '): ' num2str(nVentricle) ' voxels (>' num2str(T) ') ' num2str(vVentricle) ' cm3   center1=' num2str(cVentricle,'%.2f')]); % Threshold
                    % if the centroid of the largest ROI is too high, go to
                    % the 2nd larget ROI
                    if cVentricle > 0.65
                        kk = 2;
                        nVentricle = nCSF(kk); 
                        vVentricle = round(vVox*nVentricle); % in cm3
                        cVentricle = st.Centroid(indCSF(kk),3)/maskHeader.dim(3); 
                        disp(['Ventricle (eroded ' num2str(cnt) '): ' num2str(nVentricle) ' voxels (>' num2str(T) ') ' num2str(vVentricle) ' cm3   center2=' num2str(cVentricle,'%.2f')]);
                    end
                    maskVentricle = zeros(maskHeader.dim); 
                    maskVentricle(st.VoxelIdxList{indCSF(kk)}) = 1; 
                    ...
                    SubjectCovariatesROI=[SubjectCovariatesROI;{fname}];
                end
            end
